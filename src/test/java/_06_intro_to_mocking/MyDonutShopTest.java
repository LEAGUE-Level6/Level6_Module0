...
This leads us to a problem when testing methods that require external objects to work.  In MyDonutShop, we want to ensure
that our methods work, but they rely on the DonutShop and Orders classes to function.  We DO NOT want to write unit tests
that rely on the validity of those classes, as again, we are currently only testing MyDonutShop.

The answer: Mocking

We can create mocks of any external objects that we need to use, in this case DonutShop and Order.  These mocks represent
their respective objects, but do not actually do anything when methods are called.  Mockito accomplishes this by using
reflection to inspect these objects and create fake objects that return default/null values.  This is great because we
can now test that MyDonutShop is correct, without relying on the validity of the other objects.
